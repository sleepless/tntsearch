...
class TNTSearch
{
    ...
    public function getAllDocumentsForWhereKeywordNot($keyword, $noLimit = false)
    {
        ...
        $query = "SELECT * FROM " . $this->prefix . "doclist WHERE doc_id NOT IN (SELECT doc_id FROM " . $this->prefix . "doclist WHERE term_id = :id) GROUP BY doc_id ORDER BY hit_count DESC LIMIT {$this->maxDocs}";
        if ($noLimit) {
            $query = "SELECT * FROM " . $this->prefix . "doclist WHERE doc_id NOT IN (SELECT doc_id FROM " . $this->prefix . "doclist WHERE term_id = :id) GROUP BY doc_id ORDER BY hit_count DESC";
        }
    ...
    {
        $searchWordlist = "SELECT * FROM " . $this->prefix . "wordlist WHERE term like :keyword LIMIT 1";
        $stmtWord       = $this->index->prepare($searchWordlist);

        if ($this->asYouType && $isLastWord) {
            $searchWordlist = "SELECT * FROM " . $this->prefix . "wordlist WHERE term like :keyword ORDER BY length(term) ASC, num_hits DESC LIMIT 1";
            $stmtWord       = $this->index->prepare($searchWordlist);
    ...
    public function fuzzySearch($keyword)
    {
        ...
        $searchWordlist = "SELECT * FROM " . $this->prefix . "wordlist WHERE term like :keyword ORDER BY num_hits DESC LIMIT {$this->fuzzy_max_expansions}";
        $stmtWord       = $this->index->prepare($searchWordlist);
    ...
    {
        $query = "SELECT * FROM " . $this->prefix . "info WHERE `key` = '$value'";
    ...
    {
        $query = "SELECT * FROM " . $this->prefix . "filemap WHERE id in (".$docs->implode(', ').");";
    ...
    private function getAllDocumentsForFuzzyKeyword($words, $noLimit)
    {
        ...
        $query          = "SELECT * FROM " . $this->prefix . "doclist WHERE term_id in ($binding_params) ORDER BY hit_count DESC LIMIT {$this->maxDocs}";
        if ($noLimit) {
            $query = "SELECT * FROM " . $this->prefix . "doclist WHERE term_id in ($binding_params) ORDER BY hit_count DESC";
        }
        $stmtDoc = $this->index->prepare($query);
    ...
    {
        $query = "SELECT * FROM " . $this->prefix . "doclist WHERE term_id = :id ORDER BY hit_count DESC LIMIT {$this->maxDocs}";
        if ($noLimit) {
            $query = "SELECT * FROM " . $this->prefix . "doclist WHERE term_id = :id ORDER BY hit_count DESC";
        }
        $stmtDoc = $this->index->prepare($query);
